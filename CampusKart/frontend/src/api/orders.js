...
function extractOrderItems(order) {
  const directCandidates = [order?.items, order?.order_items, order?.line_items, order?.orderItems]
...
function normalizeOrderItem(item, index = 0) {
  ...
  const rawProductId = pickFirst(
    item?.product_id,
    product?.id,
    typeof item?.product === 'string' || typeof item?.product === 'number' ? item.product : null,
  )

  const quantity = Math.max(1, Math.floor(toNumber(pickFirst(item?.quantity, item?.qty, item?.count, 1))))
  ...
  return {
    ...
    name: asString(pickFirst(item?.product_name, item?.name, product?.name), 'Product'),
    imageUrl: resolveImageUrl(product, item),
    quantity,
    unitPrice,
    stock: toNumber(pickFirst(item?.stock, product?.stock, 0)),
    subtotal: toNumber(pickFirst(item?.subtotal, item?.line_total, item?.total, unitPrice * quantity)),
  }
}
...
function normalizeAddress(addressSource, order) {
  ...
  const fullName = asString(
    pickFirst(source?.full_name, source?.fullName, source?.name, order?.customer_name, order?.full_name),
    '',
  )
  const phone = asString(pickFirst(source?.phone, source?.phone_number, order?.phone, order?.phone_number), '')
  const addressLine = asString(
    pickFirst(source?.address_line, source?.line1, source?.address, order?.address_line, order?.address),
    '',
  )
  const areaCity = asString(
    pickFirst(source?.area_city, source?.city, source?.area, source?.district, order?.area_city, order?.city),
    '',
  )
  const notes = asString(pickFirst(source?.notes, source?.delivery_notes, order?.notes, order?.delivery_notes), '')

  const display = [fullName, phone, addressLine, areaCity].filter(Boolean).join(', ')
...
function normalizeOrder(rawOrder, index = 0) {
  ...
  const items = extractOrderItems(order).map((item, itemIndex) => normalizeOrderItem(item, itemIndex))
  const backendOrderId = pickFirst(order?.id, order?.order_id, order?.pk, null)

  const computedItemCount = items.reduce((sum, item) => sum + toNumber(item.quantity), 0)
  const computedSubtotal = items.reduce((sum, item) => sum + toNumber(item.subtotal), 0)

  const itemCountCandidate = toNumber(
    pickFirst(order?.item_count, order?.items_count, order?.total_items, order?.total_quantity, computedItemCount),
  )

  const subtotal = toNumber(
    pickFirst(order?.subtotal, order?.sub_total, order?.items_total, order?.item_total, computedSubtotal),
  )
  ...
  const orderNumber = asString(
    pickFirst(order?.order_number, order?.orderNumber, order?.number, order?.order_no, order?.id, `order-${index + 1}`),
    '',
  )
  ...
  return {
    ...
    orderNumber,
    status: normalizeStatus(pickFirst(order?.status, order?.order_status, order?.state)),
    paymentMethod: normalizePaymentMethod(
      pickFirst(order?.payment_method, order?.paymentMethod, order?.payment_type, order?.method),
    ),
    placedAt: pickFirst(order?.created_at, order?.placed_at, order?.order_date, order?.date, order?.createdAt, null),
...
function normalizeOrdersList(payload, fallbackPage = 1) {
  const orders = extractArray(payload).map((order, index) => normalizeOrder(order, index))
  const countCandidate = toNumber(
    pickFirst(payload?.count, payload?.total_count, payload?.total, payload?.pagination?.count, orders.length),
  )
  ...
  const pageSize = Math.max(
    1,
    Math.floor(toNumber(pickFirst(payload?.page_size, payload?.per_page, payload?.limit, orders.length || 10))),
  )
...
function buildCreateOrderPayloadCandidates(payload = {}) {
  ...
  const phone = pickFirst(deliveryAddressSource?.phone, deliveryAddressSource?.phone_number, source?.phone)
...
export async function createOrder(payload) {
  const response = await postWithPayloadFallback('/orders/', buildCreateOrderPayloadCandidates(payload))
...
export async function fetchOrderDetail(orderNumberOrId) {
  ...
  const detailEndpoints = [`/orders/${encoded}/`, `/orders/by-number/${encoded}/`, `/orders/detail/${encoded}/`]
  ...
  const matched =
    normalized.results.find((order) => String(order.orderNumber) === String(orderNumberOrId)) ||
    normalized.results.find((order) => String(order.id) === String(orderNumberOrId))
