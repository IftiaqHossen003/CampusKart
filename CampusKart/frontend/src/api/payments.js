...
function normalizePayoutList(payload, fallbackPage = 1) {
  ...
  const countCandidate = toNumber(
    pickFirst(payload?.count, payload?.total, payouts.length),
  );
  ...
  const pageSize = Math.max(
    1,
    Math.floor(toNumber(pickFirst(payload?.page_size, payload?.per_page, 20))),
  );
...
export async function fetchPayouts({ page } = {}) {
  ...
  const response = await apiClient.get(
    `/payments/payouts/${queryString ? `?${queryString}` : ""}`,
  );
...
export async function markPayoutPaid(payoutId, payoutReference) {
  ...
  const response = await apiClient.patch(
    `/payments/payouts/${payoutId}/mark-paid/`,
    payload,
  );
