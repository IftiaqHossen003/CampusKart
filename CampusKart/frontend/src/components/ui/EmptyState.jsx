function EmptyState({ title, message, actionLabel, onAction }) {
  return (
    <div className="rounded-xl border border-dashed border-white/20 bg-[var(--ck-surface)] p-8 text-center">
      <svg
        viewBox="0 0 240 180"
        className="mx-auto h-36 w-48"
        role="img"
        aria-label="Empty results"
      >
        <rect x="30" y="40" width="180" height="100" rx="14" fill="#1C1C1C" />
        <rect x="46" y="56" width="148" height="12" rx="6" fill="#3A3A3A" />
        <rect x="46" y="78" width="98" height="12" rx="6" fill="#3A3A3A" />
        <circle cx="184" cy="124" r="18" fill="#C8FF2F" />
        <line x1="197" y1="137" x2="214" y2="154" stroke="#C8FF2F" strokeWidth="6" strokeLinecap="round" />
      </svg>
      <h3 className="mt-4 text-lg font-semibold text-white">{title}</h3>
      <p className="mx-auto mt-2 max-w-md text-sm text-slate-600">{message}</p>
      {actionLabel && onAction ? (
        <button
          type="button"
          onClick={onAction}
          className="mt-5 rounded-md bg-[var(--ck-accent)] px-4 py-2 text-sm font-semibold text-[#111111] hover:bg-[var(--ck-accent-hover)]"
        >
          {actionLabel}
        </button>
      ) : null}
    </div>
  )
}

export default EmptyState



