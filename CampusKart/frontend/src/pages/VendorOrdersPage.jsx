...
import { fetchVendorOrders, getOrderApiErrorMessage, updateOrderStatus } from '../api/orders'
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={`vendor-orders-skeleton-${index}`} className="h-14 animate-pulse rounded bg-slate-200" />
            ))}
          </div>
        </div>
      </section>
  ...
  if (vendorOrdersQuery.isError) {
    return (
      <section className="rounded-xl border border-slate-200 bg-white p-8 text-center">
        <h1 className="text-xl font-semibold text-primary">Could not load vendor orders</h1>
        <p className="mt-2 text-sm text-muted">
          {getOrderApiErrorMessage(vendorOrdersQuery.error, 'Please try again in a moment.')}
        </p>
        <button
          type="button"
  ...
  if (orders.length === 0) {
    return (
      <section className="space-y-5">
        <div className="rounded-xl border border-slate-200 bg-white px-5 py-5 sm:px-6">
          <h1 className="text-2xl font-bold text-primary">Vendor Orders</h1>
          <p className="mt-1 text-sm text-muted">Manage incoming orders from students.</p>
        </div>
    ...
    <section className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-white px-5 py-5 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-primary">Vendor Orders</h1>
            <p className="mt-1 text-sm text-muted">Track and update the status of customer orders.</p>
          </div>

          <label className="flex items-center gap-2 text-sm text-slate-700" htmlFor="vendor-order-status-filter">
            <span>Status</span>
            <select
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-muted">Current</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-muted">Update Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 bg-white">
              {orders.map((order) => {
                ...
                const canChangeStatus = transitionOptions.length > 1 && Boolean(targetOrderId)
                const isRowUpdating = String(updatingOrderId) === String(targetOrderId)

                return (
                  <tr key={order.id || order.orderNumber}>
                    <td className="px-4 py-3 text-sm font-medium text-slate-800">#{order.orderNumber || order.id}</td>
                    <td className="px-4 py-3 text-sm text-slate-700">{order.customerName || 'Customer'}</td>
                    ...
                    <td className="px-4 py-3 text-sm">
                      ...
                      >
                        {transitionOptions.map((status) => (
                          <option key={`${targetOrderId}-${status}`} value={status}>
                            {formatLabel(status)}
                          </option>
                        ))}
                      </select>
          ...
          {orders.map((order) => {
            ...
            return (
              <article key={order.id || order.orderNumber} className="space-y-3 p-4">
                ...
                >
                  {transitionOptions.map((status) => (
                    <option key={`${targetOrderId}-mobile-${status}`} value={status}>
                      {formatLabel(status)}
                    </option>
                  ))}
                </select>
