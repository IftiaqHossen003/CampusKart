...
function OrderDetailPage() {
  ...
  const reorderMutation = useMutation({
    ...
    onSuccess: ({ successCount, failedCount, firstError }) => {
      ...
      showError(getOrderApiErrorMessage(firstError, 'Could not re-order these items.'))
    },
    onError: (error) => {
      showError(getOrderApiErrorMessage(error, 'Could not re-order these items.'))
    },
  })
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-5">
          <div className="space-y-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={`order-detail-skeleton-${index}`} className="h-12 animate-pulse rounded bg-slate-200" />
            ))}
          </div>
        </div>
      </section>
    ...
    return (
      <section className="rounded-xl border border-slate-200 bg-white p-8 text-center">
        ...
        <p className="mt-2 text-sm text-muted">
          {getOrderApiErrorMessage(orderQuery.error, 'We could not load this order right now.')}
        </p>
        <Link
          to="/orders"
    ...
    <section className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-primary">Order #{order.orderNumber || order.id}</h1>
          <p className="mt-1 text-sm text-muted">Placed on {formatDate(order.placedAt)}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[2fr_1fr]">
        <div className="space-y-5">
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            <h2 className="text-base font-semibold text-primary">Order Progress</h2>
          ...
          <div className="rounded-xl border border-slate-200 bg-white">
            ...
            <div className="divide-y divide-slate-100">
              {(order.items || []).map((item) => (
                <article key={item.id} className="grid grid-cols-[64px_1fr_auto] gap-3 px-4 py-4 sm:grid-cols-[72px_1fr_auto]">
                  <img
                    src={item.imageUrl}
                    alt={item.name}
                  ...
                  <p className="whitespace-nowrap text-sm font-semibold text-slate-800">{formatPrice(item.subtotal)}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <aside className="space-y-5">
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            <h2 className="text-base font-semibold text-primary">Order Details</h2>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            <h2 className="text-base font-semibold text-primary">Delivery Address</h2>
            <div className="mt-3 space-y-1 text-sm text-slate-700">
              {order.deliveryAddress?.fullName ? <p>{order.deliveryAddress.fullName}</p> : null}
              {order.deliveryAddress?.phone ? <p>{order.deliveryAddress.phone}</p> : null}
              {order.deliveryAddress?.addressLine ? <p>{order.deliveryAddress.addressLine}</p> : null}
              {order.deliveryAddress?.areaCity ? <p>{order.deliveryAddress.areaCity}</p> : null}
              {order.deliveryAddress?.notes ? (
                <p className="pt-1 text-xs text-muted">Notes: {order.deliveryAddress.notes}</p>
              ) : null}
              {!order.deliveryAddress?.display ? <p className="text-sm text-muted">No address details available.</p> : null}
            </div>
          </div>
